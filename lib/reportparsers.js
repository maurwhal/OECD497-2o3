...
(function (global) {
  ...
  function applyToState(found, ctx) {
    ...
    if (found.assay === 'DPRA' && found.runs && found.runs.length) {
      ...
      // partial data was recoverable; leave the rest for manual entry. Cys+Lys
      // both present -> calcDPRA derives the mean itself; Cys with no Lys ->
      // treated as co-elution (Cys-only model), matching the IIVS report itself.
      if (chosen.cys !== null && chosen.cys !== undefined) d.cysDepletion = chosen.cys;
      if (chosen.lys !== null && chosen.lys !== undefined) d.lysDepletion = chosen.lys;
      if ((chosen.cys === null || chosen.cys === undefined) && chosen.avg !== null && chosen.avg !== undefined) d.meanDepletion = chosen.avg;
      update();
    }
  }
